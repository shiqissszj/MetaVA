...
class MAML(nn.Module):
    def __init__(self, model, data, label, innerlr=maml['innerlr'], outerlr=maml['outerlr'], update=maml['update'],
        ...
        # self.train_x = data[:90]
        # self.train_y = label[:90]
        # self.valid_x = data[91:120]
        # self.valid_y = label[91:120]
    ...
    def init_difficulties(self):
        """
        对每个任务（这里 train_y 的长度=任务数，一人=一个任务）计算难度值 V_b.
        ...
        for idx in range(len(self.train_y)):
            ...
            loss = self.calculate_loss(train_data, train_label)
            Lloss.append(loss)
    ...
    # --------------- 每次外循环都要调用 "SampleTasks(iteration)" => B 个任务 (Algorithm 2) ---------------
    ...
    def metatrain(self, iteration=0):
        """
        CL选择器一次选 B 个任务，分别做 inner-loop，再累积到 outer-loop 的 loss
        """
    ...
    def valid_per_task(self, data, label):
        ...
        # 5) Few-step Inner Loop：只做 maml['valid_step'] 次更新
