...
def read_data(filename):
    ...
    ** Description **
    ...
    data = np.load(f'data0/{filename}_data.npy')  # 读取ECG数据
    pid = np.load(f'data0/{filename}_pid.npy')  # 读取患者ID
    label = np.load(f'data0/{filename}_label.npy', allow_pickle=True)  # 读取标签
