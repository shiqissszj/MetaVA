...
class Conv1dMTL(nn.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride, groups):
        super(Conv1dMTL, self).__init__(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size,
    ...
    def forward(self, x):
        ...
        if self.bias is not None:
            ...
            new_bias = None
        return nn.functional.conv1d(x, weight=new_weight, bias=new_bias, stride=self.stride, padding=self.padding,
...
class Net1D(nn.Module):
    def __init__(self, in_channels, base_filters, ratio, filter_list, m_blocks_list, kernel_size, stride, groups_width,
        ...
        self.mtl = mtl
        self.Conv = Net1DnoFC(in_channels, base_filters, ratio, filter_list, m_blocks_list, kernel_size, stride,
                              groups_width,
    ...
    def forward(self, x):
        # print(f"Input shape: {x.shape}")  # 打印输入数据的形状
        out = self.Conv(x)
        # print(f"After first conv: {out.shape}")
        # 全局平均池化
        out = out.mean(-1)
        # print(f"After second conv: {out.shape}")
        # 检查 self.dense 是否存在
        if self.dense is not None:
            out = self.dense(out)
            # print(f"After last conv: {out.shape}")
        else:
            print("No dense layer, returning pooled features")
        return out  # 确保返回张量
